...
function groupObjects(items) {
  ...
  for (const obj of items) {
    if (categoryTable.hasOwnProperty(obj.category.toLowerCase())) {
      //we're adding to the array
      categoryTable[obj.category.toLowerCase()].push(obj);
    } else {
      //we need to add to the category table and make sure the key has an array of objects
      categoryTable[obj.category] = [obj];
    }
  }
